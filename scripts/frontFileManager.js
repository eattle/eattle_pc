function frontFileManager() {
  this.filesystem = new fileSystem(); //그냥쓰고싶을떄 
  this.filesystemlayer = new fileSystemLayer(this.filesystem);// *쓰고싶을 떄 
};
...
frontFileManager.prototype = {
  ...
  fileViewInit : function(){
    ...
    this.filesystemlayer.fileSystemInit(function () {
      ...
      //progressbar 
      self.leftFolderAdd(function(err){
        self.centerFolderAdd(function(err){
          $('#loadingBar').hide();
        });
      ...
      $('#log').bind('click',function () {
        ...
        if(self.isClickfolder == true){ //폴더 
          ...
          if(self.currentTargetIdCount === 0){
            self.filesystemlayer.filedelete(self.currentTargetId,function(err){
  ...
  if(self.currentTargetIdCount === 0)
    self.filesystemlayer.fileout(self.currentTargetId,false,function(err){
      $('#loadingBar').hide();
    });
  else{
    for(var i=0;i<self.currentTargetIdCount;i++){
...
*/
      ...
      $('#moreoption').bind('click',function () {
        if(self.moreoptionToggle)
          self.moreoptionToggle = false;
        else
          self.moreoptionToggle = true;
  ...
  var makeThumbnail = function (filename) {
    ...
    fs.exists('thumbnails/'+ filename, function (exists) {
      if(exists){
        ...
        self.filesystemlayer.fileout(filename,"originals",function(err){
...
/*
  ...
  */
  ...
  calculatePictureInterval : function(){ //사진간 시간 간격을 계산하는 함수
    var self = this;
    ...
    var db = new sqlite3.Database('CaPicDB');
        ...
        db.all("SELECT * FROM media ORDER BY picturetaken ASC", function(err, rows) {
          for(var i =0;i< rows.length;i++){
              ...
              pictureTakenTime = _pictureTakenTime;
              self.totalPictureNum++; //갯수
            }
            console.log("차이 : "  + self.totalInterval  + " 갯수 : "  + self.totalPictureNum);
          ...
          for(var i =0;i< rows.length;i++){
            ...
            if (pictureTakenTime - _pictureTakenTime > averageInterval && _pictureTakenTime != 0) {
              ...
              console.log(folderIDForDB + " " + new_name)
                ...
                // id가 저장되있는거랑 같은파일 picturetaken  id에 folderIDForDB를 넣기 

                for(var j=0; j<fileIdcount; j++){
